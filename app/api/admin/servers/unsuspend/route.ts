...
import { unsuspendServer } from '@/lib/pterodactyl'
...
export async function POST(request: Request) {
  try {
    ...
    await query(
      'UPDATE servers SET status = ?, suspended_at = NULL WHERE id = ?',
      ['active', serverId]
    )

    // Unsuspend on Pterodactyl if configured
    if (server?.pterodactyl_id) {
      try {
        await unsuspendServer(server.pterodactyl_id)
      } catch (pteroError) {
        console.error('Failed to unsuspend on Pterodactyl:', pteroError)
      }
    }
